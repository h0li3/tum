...
int bx_gdbstub_check(bx_address addr)
{
  ...
  for (i = 0; i < nr_breakpoints; i++)
  {
    ...
    {
      BX_INFO(("found breakpoint at %x", addr));
      last_stop_reason = GDBSTUB_EXECUTION_BREAKPOINT;
      return GDBSTUB_EXECUTION_BREAKPOINT;
    }
  }
...
static void debug_loop(void)
{
    ...
    while (ne == 0) {
        ...
        // ‘m addr,length?      // Read length bytes of memory starting at address addr. Note that addr may
        ...
        case 'm':
        {
            ...
            if (access_linear(addr, len, BX_READ, mem)) {
...
#define PUTREG(buf, val, len) do { \
        ...
        case '?':
            ...
            // ‘H op thread-id?      // Set thread for subsequent operations (‘m? ‘M? ‘g? ‘G? et.al.). op depends on the
        ...
        case 'H':
            ...
            // ‘q name params...?      // ‘Q name params...?      // General query (‘q? and set (‘Q?. These packets are described fully in
        ...
        case 'q':
            ...
            else if (strncmp(&buffer[1], "GetTIBAddr", strlen("GetTIBAddr")) == 0) {
#if BX_SUPPORT_X86_64
                bx_address tib = bx_cpu.sregs[BX_SEG_REG_GS].cache.u.segment.base;
#else
                bx_address tib = bx_cpu.sregs[BX_SEG_REG_FS].cache.u.segment.base;
#endif
                mem2hex((Bit8u*)&tib, obuf, sizeof(tib));
                printf("Get TIB address: %llx\n", tib);
                put_reply(obuf);
            }
            else {
            ...
            // ‘z type,addr,kind?      // ‘Z type,addr,kind?      // Insert (‘Z? or remove (‘z? a type breakpoint or watchpoint starting at address
