...
int main(int argc, char** argv)
{
	...
	bx_cpu.cr0.set_PG(0);	// 开启分页模式后，所有内存访问会先经过MMU将线性地址翻译为物理地址，如果访问的4k页面不存在，就会产生#PF(page fault)异常，内存管理器负责判断这个4k页面是否是已经分配的内存，并进行页面分配或者结束执行报告内存错误
