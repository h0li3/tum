...
{
#if BX_SUPPORT_SMP
  nthreads = 4;//SIM->get_param_num(BXPN_CPU_NTHREADS)->get();
  ncores = 4;//SIM->get_param_num(BXPN_CPU_NCORES)->get();
  nprocessors = 1;//SIM->get_param_num(BXPN_CPU_NPROCESSORS)->get();
#else
  nthreads = 1;
  ncores = 1;
  nprocessors = 1;
