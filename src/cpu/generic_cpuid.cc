...
{
  static const char *brand_string = "nothing";

  static bool cpuid_limit_winnt = false;//SIM->get_param_bool(BXPN_CPUID_LIMIT_WINNT)->get();
...
{
  static const Bit8u *vendor_string = (const Bit8u*)"nothing";
...
void bx_generic_cpuid_t::init_cpu_extensions_bitmask(void)
{
...
#if BX_CPU_LEVEL >= 4
  static unsigned cpu_level = 5;//SIM->get_param_num(BXPN_CPUID_LEVEL)->get();

  enable_cpu_extension(BX_ISA_486);

#if BX_CPU_LEVEL >= 5
  static bool mmx_enabled = true;//SIM->get_param_bool(BXPN_CPUID_MMX)->get();
  ...
  static unsigned simd_enabled = BX_CPUID_SUPPORT_SSE;//SIM->get_param_enum(BXPN_CPUID_SIMD)->get();
  ...
  static bool sse4a_enabled = false;//SIM->get_param_bool(BXPN_CPUID_SSE4A)->get();
  if (sse4a_enabled) {
  ...
  static bool misaligned_sse_enabled = false;//SIM->get_param_bool(BXPN_CPUID_MISALIGNED_SSE)->get();
  ...
  static bool sep_enabled = false;//SIM->get_param_bool(BXPN_CPUID_SEP)->get();
  if (sep_enabled) {
  ...
  static bool xsave_enabled = false;//SIM->get_param_bool(BXPN_CPUID_XSAVE)->get();
  if (xsave_enabled) {
  ...
  static bool xsaveopt_enabled = false;//SIM->get_param_bool(BXPN_CPUID_XSAVEOPT)->get();
  if (xsaveopt_enabled) {
  ...
  static bool aes_enabled = false;//SIM->get_param_bool(BXPN_CPUID_AES)->get();
  if (aes_enabled) {
  ...
  static bool sha_enabled = false;//SIM->get_param_bool(BXPN_CPUID_SHA)->get();
  if (sha_enabled) {
  ...
  static bool movbe_enabled = false;//SIM->get_param_bool(BXPN_CPUID_MOVBE)->get();
  if (movbe_enabled) {
  ...
  static bool adx_enabled = false;//SIM->get_param_bool(BXPN_CPUID_ADX)->get();
  if (adx_enabled) {
  ...
  static bool x86_64_enabled = true;//SIM->get_param_bool(BXPN_CPUID_X86_64)->get();
  if (x86_64_enabled) {
    if (cpu_level < 6) {
    ...
    static bool fsgsbase_enabled = false;//SIM->get_param_bool(BXPN_CPUID_FSGSBASE)->get();
    if (fsgsbase_enabled)
      enable_cpu_extension(BX_ISA_FSGSBASE);

    static unsigned apic_enabled = 0;//SIM->get_param_enum(BXPN_CPUID_APIC)->get();
    if (apic_enabled < BX_CPUID_SUPPORT_XAPIC) {
      BX_PANIC(("PANIC: x86-64 emulation requires XAPIC support !"));
      return;
    }

    static bool pcid_enabled = false;//SIM->get_param_bool(BXPN_CPUID_PCID)->get();
    if (pcid_enabled)
      enable_cpu_extension(BX_ISA_PCID);

    static bool xlarge_pages = false;//SIM->get_param_bool(BXPN_CPUID_1G_PAGES)->get();
    if (xlarge_pages)
      enable_cpu_extension(BX_ISA_1G_PAGES);
  }
  else {
    static unsigned vmx_enabled = false;//SIM->get_param_num(BXPN_CPUID_VMX)->get();
    if (vmx_enabled >= 2) {
  ...
  static bool avx_f16c_enabled = false;//SIM->get_param_bool(BXPN_CPUID_AVX_F16CVT)->get();
  if (avx_f16c_enabled) {
    if (simd_enabled < BX_CPUID_SUPPORT_AVX) {
  ...
  static bool avx_fma_enabled = false;//SIM->get_param_bool(BXPN_CPUID_AVX_FMA)->get();
  if (avx_fma_enabled) {
  ...
  static unsigned bmi_enabled = false;//SIM->get_param_num(BXPN_CPUID_BMI)->get();
  if (bmi_enabled) {
  ...
  static bool fma4_enabled = false;//SIM->get_param_bool(BXPN_CPUID_FMA4)->get();
  if (fma4_enabled) {
    if (simd_enabled < BX_CPUID_SUPPORT_AVX) {
  ...
  static bool xop_enabled = false;//SIM->get_param_bool(BXPN_CPUID_XOP)->get();
  if (xop_enabled) {
    if (simd_enabled < BX_CPUID_SUPPORT_AVX) {
  ...
  static bool tbm_enabled = false;//SIM->get_param_bool(BXPN_CPUID_TBM)->get();
  if (tbm_enabled) {
...
#if BX_SUPPORT_SVM
  static unsigned svm_enabled = false;//SIM->get_param_num(BXPN_CPUID_SVM)->get();
  if (svm_enabled) {
  ...
  static bool smep_enabled = false;//SIM->get_param_bool(BXPN_CPUID_SMEP)->get();
  if (smep_enabled) {
  ...
  static bool smap_enabled = false;//SIM->get_param_bool(BXPN_CPUID_SMAP)->get();
  if (smap_enabled) {
...
#if BX_SUPPORT_X86_64
    static bool x86_64_enabled = false;//SIM->get_param_bool(BXPN_CPUID_X86_64)->get();
    if (x86_64_enabled) {
...
void bx_generic_cpuid_t::init_svm_extensions_bitmask(void)
{
  ...
  static bool svm_enabled = false;//SIM->get_param_bool(BXPN_CPUID_SVM)->get();
  if (svm_enabled) {
...
{
  static Bit32u level = 5;//SIM->get_param_num(BXPN_CPUID_LEVEL)->get();
  static Bit32u stepping = 0;//SIM->get_param_num(BXPN_CPUID_STEPPING)->get();
  static Bit32u model = 0;//SIM->get_param_num(BXPN_CPUID_MODEL)->get();
  static Bit32u family = 0;//SIM->get_param_num(BXPN_CPUID_FAMILY)->get();
